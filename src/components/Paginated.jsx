...
function Paginated({ current, total, handle }) {
  ...
  return (
    <Pagination className="justify-content-center mt-4">
      ...
      <Pagination.Prev
        onClick={() => handle(current > 1 ? current - 1 : 1)}
        disabled={current === 1}
      />
      ...
      {current > 1 && (
        <Pagination.Item onClick={() => handle(current - 1)}>
          {current - 1}
        </Pagination.Item>
      )}
      ...
      {current < total - 2 && (
        <>
          <Pagination.Ellipsis />
          <Pagination.Item onClick={() => handle(total)}>
            {total}
          </Pagination.Item>
        </>
      )}
      ...
      <Pagination.Next
        onClick={() => handle(current < total ? current + 1 : total)}
